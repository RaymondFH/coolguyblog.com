...
const gameLoop = setInterval(() => {
  ...
  const blockRight =
    gameWidth -
    parseFloat(window.getComputedStyle(block).getPropertyValue("right"));
  const blockWidth = parseFloat(
  ...
  const isInBlockRange =
    blockRight <= gameWidth * 0.2 && blockRight + blockWidth >= gameWidth * 0.2;
